...
export class ApiErrorHandler {
  ...
  handleError(err: any, skipDefaultErrorHandler?: boolean): Observable<ApiError> {
    for (const noSkipErrorHandler of this.noSkipErrorHandlers) {
      const handledError = noSkipErrorHandler(err);
      if (handledError) {
        return handledError;
      }
    }
    ...
    if (!skipDefaultErrorHandler) {
      for (const errorHandlerItem of this.errorHandlers) {
        const handledError = errorHandlerItem(err);
        if (handledError) {
          return handledError;
        }
      }
