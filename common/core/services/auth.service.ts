...
import { TokenModel } from 'common/structures';
...
export class AuthService {
  ...
  loginByToken(tokenType: TokenModel, token: string): Observable<any> {
    return this.apiClientService.call(REQUESTS_DEFINITION.KEY_LOGIN_BY_TOKEN, {
      payload: {
        tokenType: tokenType,
        token: token,
      },
      ...
      map((response: any) => {
        this.mapResponseFields(response);
        this.authenticateEmitter.next('token');
